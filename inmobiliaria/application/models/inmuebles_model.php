...
class Inmuebles_model extends CI_Model {
    ...
    public function verInmuebleTablaAgregarImagenes(){
        $sql = " SELECT E_ID, T_TITULO, T_ESTADO, T_MUNICIPIO, T_COLONIA FROM TA02_CAT_INMUEBLES ";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    
    public function buscarFoto($T_NOMBRE_FOTO, $E_ID){
        $sql = " SELECT * FROM TA09_CTRL_FOTOS_INMUEBLES 
                 WHERE T_NOMBRE_FOTO = '".$T_NOMBRE_FOTO."' AND TA02_E_ID_INMUEBLE = ".$E_ID;
        $query = $this->db->query($sql);
        return $query->result_array();
        
    }

    public function guardarFotoInmueble($T_NOMBRE_FOTO, $E_ID){
        $sql = " INSERT INTO TA09_CTRL_FOTOS_INMUEBLES (T_NOMBRE_FOTO, TA02_E_ID_INMUEBLE) 
                 VALUES ('".$T_NOMBRE_FOTO."','$E_ID') ";
        $query = $this->db->query($sql);
